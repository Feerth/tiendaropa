...
export function WhatsAppButton({ message, className = "", phoneNumber }: WhatsAppButtonProps) {
  const number = phoneNumber || process.env.NEXT_PUBLIC_WHATSAPP_NUMBER || "51931869696";
  const defaultMessage = "¡Hola! Quiero consultar sobre sus productos.";
  const url = `https://wa.me/${number}?text=${encodeURIComponent(message || defaultMessage)}`;
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 animate-fade-in-up">
      ...
      <a
        ...
        target="_blank"
        rel="noopener noreferrer"
        className="md:hidden flex items-center gap-2 pl-4 pr-5 h-12 bg-[#25D366] rounded-full shadow-lg shadow-[#25D366]/30 hover:scale-105 active:scale-95 transition-all duration-200"
      >
